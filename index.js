...
import { PROXY_LIST } from "./proxy/ip/proxy_https (mikrotik)_1.js";
...
try {
  ...
  const previousReports = files.filter(
    (file) => file.startsWith("report-") && file.endsWith(".json")
  );

  if (previousReports.length > 0) {
    console.log(`Found ${previousReports.length} previous report(s)`);
  }
} catch (error) {
